...
import { MapPin, Plus, Loader2, Clock, Search, X } from 'lucide-react';
...
export default function Sidebar({ token, activeConversationId, refreshKey, onSelect, onNewChat, mobileOpen, onMobileClose }) {
  ...
  const sidebarContent = (isMobile = false) => (
    <>
      ...
      <div className="flex-1 overflow-y-auto pb-4">
        ...
        ) : conversations.length === 0 && (isMobile || !collapsed) ? (
          <div className="px-3 mt-3">
            <p className="text-xs text-ramp-text-tertiary">No trips yet. Start planning!</p>
          </div>
        ) : filteredConversations.length === 0 && (isMobile || !collapsed) ? (
          <div className="px-3 mt-3">
          ...
          filteredConversations.map((c) => {
            ...
            return (
              <button
                ...
                title={(!isMobile && collapsed) ? (c.title || 'Untitled Trip') : undefined}
                className={`w-full text-left flex items-center gap-2.5 transition-colors
                  ${(!isMobile && collapsed) ? 'justify-center px-0 py-3' : 'px-3 py-2.5'}
                  ${isActive
  ...
  return (
    <>
      ...
      <aside
        className={`hidden md:flex flex-shrink-0 bg-ramp-bg border-r border-ramp-border flex-col h-full transition-all duration-200
          ${collapsed ? 'w-12' : 'w-52'}`}
      >
      ...
      {mobileOpen && (
        <div className="md:hidden fixed inset-0 z-50 flex">
          ...
          />
          {/* Drawer panel */}
          <aside className="relative z-10 w-72 max-w-[85vw] bg-ramp-bg border-r border-ramp-border flex flex-col h-full shadow-xl">
            {sidebarContent(true)}
          </aside>
        </div>
      )}
